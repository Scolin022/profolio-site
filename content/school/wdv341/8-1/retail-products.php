...
<body>
    <header>
        ...
        <?php
            ...
            // Fetch all rows in descending order of product_name
            $query = "SELECT * FROM wdv341_products ORDER BY product_name DESC";
            ...
            { 
                ...
                // Loop through the rows and display in the formatting asked
                foreach ($result as $row)  
                { 
                    echo 
                    '<div class="productBlock">',
                        '<div class="productImage">',
                            // easier way to display images based on product_id (had to rename initial jpg files)
                            '<img src="productImages/product-'.$row['product_id'].'.jpg">',
                        '</div>',
                        '<p class="productName">'.$row['product_name'].'</p>',
                        '<p class="productDesc">'.$row['product_description'].'</p>',
                        '<p class="productPrice">$'.$row['product_price'].'</p>',
                        '<p class="productStatus">'.$row['product_status'].'</p>',
                        '<p class="productInventory">'.$row['product_inStock'].'</p>',
                    '</div>';
                } 
